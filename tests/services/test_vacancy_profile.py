...
def test_portrait_fields_default_to_empty(tmp_engine):
    Base.metadata.create_all(tmp_engine)
    with Session(tmp_engine) as session:
        profile = create_vacancy_profile_version(
            session, course_id=1, requirements={"a": 1}, notes="", created_by_id=None,
        )
        assert profile.portrait_text == ""
        assert profile.source_url is None


def test_portrait_fields_are_saved(tmp_engine):
    Base.metadata.create_all(tmp_engine)
    with Session(tmp_engine) as session:
        profile = create_vacancy_profile_version(
            session, course_id=1, requirements={"a": 1}, notes="", created_by_id=None,
            portrait_text="Ищем Python-разработчика", source_url="https://example.com/vacancy",
        )
