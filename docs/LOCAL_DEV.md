...
2. `db` не публикует порт на host VPS — только внутри docker-сети `ai_shared`
   (см. `project-context/sfera-ai/03_TDD.md`, раздел «Инфраструктура и сеть»).
   Туннель поднимает временный socat-proxy контейнер на VPS внутри `ai_shared`
   и прокидывает его на localhost через `ssh -L`:
   ```bash
   VPS_HOST=<прод-IP, спросить у владельца> VPS_USER=<ssh-логин> \
