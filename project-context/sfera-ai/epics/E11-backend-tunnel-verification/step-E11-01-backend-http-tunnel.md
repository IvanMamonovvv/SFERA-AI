...
- [ ] Скрипт поднимает туннель, идемпотентен при повторном запуске
- [ ] `cleanup()` trap подтверждён — после `Ctrl+C`/штатного завершения proxy-контейнер
      на VPS реально удаляется (не остаётся висеть)
- [ ] Ручной тест: через туннель `HHClient` реально получает ответ от backend (не 404/DNS-ошибка)
