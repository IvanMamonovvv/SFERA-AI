...
**Статус:** DONE (частично — см. журнал: создание роли `ai_readonly` на проде вне доступа агента)
...
- [ ] Роль `ai_readonly` реально создана на проде (DDL из «Предпосылки»), не superuser и
      ...
      **Не выполнено агентом** — требует явного разрешения владельца на прод-БД
      (доступ к VPS у агента отсутствует). Нужно сделать перед E0-05.
...
- `2026-08-26` — реализованы `scripts/tunnel-platform-db.sh` и `docs/LOCAL_DEV.md`.
  ...
  `d6a140e`. Создание роли `ai_readonly` на проде — НЕ выполнено, вне доступа агента,
  требует владельца перед E0-05.
