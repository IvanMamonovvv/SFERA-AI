...
- 2026-09-09: реализовано в `SPHERA` с явного разрешения владельца (правка чужого репозитория).
  Подробности — журнал `step-07-hr-ui.md` внешнего плана
  (`project-context/PLATFORM_video-transcription-plan/`). Кратко: поле `transcription`
  добавлено в `CandidateAnswerItem` (`model/candidate-model.ts`), маппер
  (`lib/candidate-answers-mapper.ts`), блок «Разбор видеовизитки» — новый компонент
  `TranscriptionBlock` в `CandidateProfile.tsx` (бейдж статуса, summary, спойлер
  транскрипта, обработка пустого видео). `tsc`+`eslint` по правленым файлам — чисто.
