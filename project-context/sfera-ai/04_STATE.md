...
**Последнее обновление:** `2026-09-02` — заведён новый эпик **E13** (приоритет
источников резюме > ответы > видео + видимость «нет резюме»), план согласован
владельцем. Причина: реальный прогон E10-01/E11-02 показал, что резюме реально
меняет вывод модели, но приоритета между источниками в коде нет — все складываются
равноправно (`candidate_facts.py`), отсутствие/битое резюме нигде не сигнализируется
менеджеру. Владелец явно уточнил: это НЕ hard-block — если ответы+видео в порядке,
менеджер продолжает работать с кандидатом; нехватка данных должна органически
снижать уверенность оценки, не блокировать статусом. Решение — в памяти
[[project_source_priority_resume_answers_video]]. Два шага:
`step-E13-01-resume-priority-prompt.md` (инструкция приоритета в промпте
fit-scoring, версия промпта инкрементируется) и
`step-E13-02-resume-missing-flag.md` (новый helper `resume_status` в
`candidate_facts.py`, флаг в CSV CLI-прогона / PDF-карточке / API
`get_candidate_detail`+`list_candidates`, конкретные точки вставки найдены заранее).
После E13 — пересчитать оставшихся 84 кандидатов course_id=39 одним прогоном
(решение владельца, не дублировать работу E11-02). Код ещё не написан — ждёт
«приступай». Детали — `epics/E13-source-priority-visibility/`, `05_EPICS.md`.
...
**Эпик E13** (`step-E13-01-resume-priority-prompt.md`, потом
`step-E13-02-resume-missing-flag.md`) — план согласован владельцем 2026-09-02,
ждёт «приступай» для начала реализации. После него — пересчитать оставшихся 84
кандидата course_id=39 одним прогоном (решение владельца, привязано к E13, не
раньше него). Эпики **E11**/**E12 полностью завершены** (2026-09-01). Roadmap E0–E9
завершён; эпик **E10** (ручной прогон вакансии +
сборка портрета из ссылки, CLI без UI) **полностью завершён** (E10-01, E10-02).
E10-01 — реальный прогон на проде ещё не запускался (ждёт GRANT+подтверждения
владельца, см. журнал `step-E10-01-full-course-screening.md`).
...
| E13-01 | Приоритет резюме в промпте fit-scoring (`step-E13-01-resume-priority-prompt.md`) | TODO | — |
| E13-02 | Флаг «нет резюме» в CSV/PDF/API (`step-E13-02-resume-missing-flag.md`) | TODO | — |
