...
смыслу требования, а не скопированный verbatim snake_case-ключ из JSON вакансии.
`PROMPT_VERSION` `"fit-scoring-v3"` → `"fit-scoring-v4"`. `uv run pytest
tests/services/test_fit_scoring.py` — 10 passed, регрессий нет. Не закоммичено (вместе с
ранее не закоммиченными E19-E21). Следующий шаг — не определён владельцем, ждать новую
задачу.

Предыдущее: `2026-09-10` — шаг **E21-01** (убрать хвост ",0" в
отображении баллов PDF) выполнен, **эпик E21 полностью завершён**. Новая функция
`_format_score_value(value)` (`ai_card.py`) — ветвит по `value == int(value)` вместо
`.rstrip("0")` на строке (риск, найденный ревью-агентом: rstrip на `"10,0"` давал битый
`"10,"`). Применена в `_format_criterion_score` и `fit_score_str` (`render_ai_card_pdf`).
Принятое ограничение: старые записи с багом "100" вместо "10" (до E20) отрендерятся как
"100/10" — чище, но число всё ещё неверное, отдельный фикс не входит в этот шаг. 3 новых
теста, `uv run pytest` — 251 passed, 1 неродственный флаки-фейл
(`test_post_vacancy_profile_enqueues_full_screening_in_background`, воспроизводится и в
изоляции без правок — таймаут фоновой джобы). Не закоммичено.

Предыдущее: `2026-09-10` — шаг **E20-01** (строгая валидация диапазона
`criteria_scores`) выполнен, **эпик E20 полностью завершён**. `_parse_and_validate`
(`fit_scoring.py`) — каждое значение `criteria_scores`: `None` пропускается, число вне
`[0, 10]` или не кратное 0.5 (`value * 2 == round(value * 2)`) → `InvalidFitScoringResponse`,
без клэмпа/деления (строгий reject — не маскировать систематическую ошибку модели). Зависимость
от E19 (потолок попыток + авто-ретрай FAILED) уже закрыта. 3 новых теста
(`-k criteria_scores`), полный `uv run pytest` — 249 passed, регрессий нет. Не закоммичено.

Предыдущее: `2026-09-10` — шаг **E19-02** (авто-ретрай `AIProcessingJob.status=FAILED`)
выполнен, **эпик E19 полностью завершён**. Решение: отдельный cron `run_ai_job_retry`
(`scheduler.py`, `CronTrigger(hour="*/2", minute=30)`, офсет от `run_resume_retry` на 30
минут) по образцу `step-E18-03`, не расширение `run_tick`/`process_batch`. Новая функция
`requeue_failed_jobs(session, *, max_attempts)` (`job_processing.py`) — выборка
`status=FAILED AND attempts<max_attempts AND (retry_after IS NULL OR retry_after<=now())`;
перед флипом в `PENDING` проверяет конфликт с уже открытой (PENDING/PROCESSING) джобой той
же тройки `(candidate_profile_id, course_id, reason)` — partial unique index
`uq_processing_job_candidate_course_reason_open` — при конфликте не трогает (остаётся
FAILED, `IntegrityError` не возникает). Использует уже существующий `ai_processing_job_max_attempts`
(E19-01), нового конфига не потребовалось. 3 новых теста, `uv run pytest` — 246 passed,
регрессий нет. Не закоммичено (вместе с ранее не закоммиченным E19-01).

Предыдущее: `2026-09-10` — шаг **E19-01** (`ai_processing_job_max_attempts`,
потолок ретраев `AIProcessingJob`) выполнен. `config.py::ai_processing_job_max_attempts=5`
(по образцу `resume_extract_max_attempts`). `requeue_stuck_jobs` — новый обязательный
keyword-параметр `max_attempts`; зависшая (PROCESSING) джоба, исчерпавшая попытки после
инкремента, идёт в `FAILED` вместо очередного `PENDING`. `scheduler.py::run_requeue_stuck`
передаёт настройку. `process_batch` уже инкрементировал `attempts`/`retry_after` при
провале — подтверждено без изменений. 1 новый тест + 2 обновлённых, `uv run pytest` —
243 passed (1 неродственный флаки-тест прошёл при повторном прогоне отдельно). Не
закоммичено. Следующий шаг эпика **E19** — `E19-02` (авто-ретрай `FAILED` джоб, требует
E19-01), ждёт явного «начинай» от владельца.

Предыдущее: `2026-09-10` — владелец на реальной PDF-карточке заметил
некорректное отображение баллов критериев ("10,0/10" вместо "10/10", а иногда явный баг
"100,0/10") и нечитаемые названия критериев ("опыт_B2B_продаж"). Разбор + `architect-reviewer`
нашли: (1) `_format_criterion_score`/`fit_score_str` (`ai_card.py`) всегда режут число
`.1f`, отсюда лишний хвост ",0"; (2) `_parse_and_validate` (`fit_scoring.py`) не проверяет
диапазон значений `criteria_scores` — модель иногда путает шкалу 0-10 со шкалой `fit_score`
0-100, отсюда "100,0/10"; (3) названия критериев — сырые snake_case-ключи из
`vacancy_profile.requirements`, скопированные LLM verbatim, промпту не сказано писать
читаемо. **Критическая находка ревью-агента**: строгую валидацию (2) нельзя катить раньше
инфраструктуры ретраев — у `AIProcessingJob` сейчас нет ни потолка попыток, ни авто-ретрая
FAILED (в отличие от уже реализованного для `ResumeExtract`, E18-03) — отклонённый ответ
LLM превратит кандидата в "молча зависший без анализа навсегда", что хуже видимого бага.
Заведено **4 новых эпика** (`05_EPICS.md`): **E19** (ретраи `AIProcessingJob` — конфиг
`ai_processing_job_max_attempts` + авто-ретрай FAILED, предпосылка для E20), **E20**
(строгая валидация `criteria_scores` 0-10 в `_parse_and_validate`, зависит от E19), **E21**
(убрать хвост ",0" в `_format_criterion_score`/`fit_score_str`, независим), **E22**
(читаемые русские названия критериев в промпте + `PROMPT_VERSION` → `fit-scoring-v4`,
независим). Портрет вакансии в виде сырого JSON-textarea в HR UI (скрин владельца) — форма
находится в `FullSphera/SPHERA`, не в этом репозитории; `build_vacancy_requirements()`
(SFERA-AI, `vacancy_portrait.py`) уже принимает произвольный текст и сам строит JSON —
правка нужна только на стороне SPHERA, отложена, владелец попросил не спешить (нужно
отдельное явное разрешение на правку чужого репозитория). Реализация ни одного из E19-E22
не начата — ждёт явного «начинай» от владельца.

Предыдущее: `2026-09-10` — **Шаг C эпика E18 закрыт, эпик E18
...
платформы») — сырые данные, введённые кандидатом на регистрации, не сверяются с
резюме; (2) резюме сейчас отдельный файл в ZIP (E9-03), не приложено к самой
карточке. Решения владельца 2026-09-10: резюме подменяет платформенные поля при
успешном парсинге (не просто флаг конфликта, вариант A); резюме и карточка — один
PDF-документ через merge (`pypdf`), не отдельные файлы в ZIP. Три шага заведены
(`E17-01` подмена полей, `E17-02` починка известного бага невложения резюме —
кейс Петрусевич 2398 из записи `2026-09-06` ниже, `E17-03` склейка PDF). Реализация
**не начата** — ждёт явного «начинай» от владельца (правило проекта).

Предыдущее: `2026-09-09` — при подготовке первого прод-деплоя E16
...
нигде не был подключён к деплою ни на одном окружении. Даже при `ai_processing_dry_run=False`
реальные AI-вызовы никогда бы не запустились. Три шага, все DONE: (1) **E16-01** — новый
сервис `ai-scheduler` в `docker-compose.yml`, тот же образ, команда `python -m
sfera_ai.scheduler` вместо uvicorn; закреплён инвариант «ровно один инстанс» (комментарий в
compose + явная проверка в деплой-скрипте) — `build_scheduler()` вызывается только из
`__main__` `scheduler.py`, не из `api/app.py`. Postgres advisory lock в коде сознательно НЕ
добавлен — партиционный уникальный индекс на `AIProcessingJob` (E5-07) и `SELECT ... FOR
UPDATE SKIP LOCKED` в `process_batch` (E5-04) уже защищают от порчи данных при случайном
втором инстансе, дисциплина держится на уровне compose (без `replicas`), лишний lock был бы
защитой от уже недопустимого сценария. (2) **E16-02** — `scripts/deploy-ai-service.sh` по
образцу `sfera_backend/scripts/deploy-backend.sh`/`SPHERA/scripts/deploy-frontend.sh`: `git
pull origin main` → `docker compose up -d --build` → `alembic upgrade head` → healthcheck
`/health` → жёсткая проверка `count(ai-scheduler) == 1` (`exit 1`, если не так). (3) **E16-03**
— `docs/DEPLOYMENT.md`: состав сервиса, первый деплой, day-2 обновление, полный список env
(свои + парные `SFERA_AI_SHARED_SECRET`/`SFERA_AI_BASE_URL` в `sfera_backend` из E15-07),
явное правило про единственный инстанс планировщика. Живой прогон деплой-скрипта на VPS не
выполнялся (нет доступа поднимать прод-контейнеры без отдельного запроса владельца) — первый
реальный деплой на VPS остаётся открытым пунктом. Детали — журналы `step-E16-01/02/03-*.md`.

Предыдущее: шаг **E15-08** (HR UI модалка,
код в `SPHERA`) реализован по явному разрешению владельца, **эпик E15 полностью
завершён по коду**. По ходу закрыты (с отдельным согласованием владельца) три
расхождения дизайна с уже сданными E15-03/06/07: (1) `requirements` — API требует
`dict`, не текст, — textarea показывает/принимает JSON-текст
(`JSON.stringify`/`JSON.parse` на фронтенде), невалидный JSON — инлайн-ошибка,
без похода в API; (2) не было `GET vacancy-profile/` прокси для предзаполнения
textarea — добавлен в `VacancyProfileProxyView` (`sfera_backend`) +
`fetch_vacancy_profile` в клиенте; (3) в `GET candidates/screening/` не было
ключа для связи строки с фронтендовым `candidateStore` (имя/email) —
добавлено nullable поле `application_id` (`api_read.py`, SFERA-AI). Плюс
самостоятельно найден и исправлен баг общей инфраструктуры SPHERA: BFF-прокси
`route.ts` портил бы бинарный `application/zip` через `.text()` — исправлено на
`arrayBuffer()`. Новый домен `SPHERA`: `domains/CandidateScreening/` + виджет
`CandidateScreeningModal.tsx` (по образцу `CandidateMergeModal.tsx`), кнопка
рядом с «Выгрузить архив» в `CandidatesWorkspace.tsx`. `npx tsc --noEmit`+`eslint`
(SPHERA) чисто, `manage.py test` весь `sfera_backend` 684 passed/3 skipped,
`uv run pytest` весь `SFERA-AI` 205 passed — регрессий нет. Браузерная проверка
golden path не выполнена (нет dev-сервера в сессии) — открытый пункт для
владельца. Ничего не закоммичено ни в одном из трёх репозиториев. Детали —
журнал `step-E15-08-hr-ui-modal.md` (+ дополнения в журналах
`step-E15-03/06/07`).

Предыдущее: шаги **E15-07** (HTTP-клиент к
...
(не `PROXY_API_KEY` — значение реально от OpenRouter, не proxyapi.ru). Владелец сам
дописал `.env` на VPS и выполнил `docker compose up -d --build`. Проверено мной по
SSH (явное разрешение владельца на каждый вызов): контейнер `transcribe-worker`
поднят стабильно (0 рестартов), все нужные env внутри контейнера на месте. Живой
smoke на реальном видео на проде: `Answer.id=17576` → `TranscriptionJob` создан
вручную, воркер подхватил и довёл `PENDING→PROCESSING→DONE` за ~64с,
`transcript_text` 982 символа, `summary_text` — связный русский пересказ через
реальный вызов OpenRouter/gpt-4o-mini. Тестовая запись удалена сразу после проверки.
Платформа не пострадала (`/api/v1/docs/` → 200, все 5 контейнеров up). Детали —
журнал `step-E14-09-prod-rollout.md` и `PLATFORM_video-transcription-plan/
step-10-prod-rollout.md`/`01_STATE.md` (оба — DONE). **Следующий шаг видео-
транскрибации — E14-10** (change detection: `TranscriptionJob.status=DONE` не
попадает в `compute_current_sources_snapshot`/`CandidateProfile.facts`,
репозиторий `SFERA-AI`, свой код, без ограничения на разрешение per-шаг).

`2026-09-09` (предыдущее) — **исправление важной неточности**:
все предыдущие записи ниже про E14-01…08 утверждали «изменения НЕ закоммичены в
`sfera_backend`» — на деле весь код этих шагов давно смерджен в `main`
`FullSphera/sfera_backend` (PR #111, #114, #116, #118, #120, #124, #126), `git status`
в рабочей копии чистый. Записи ниже оставлены как есть (исторический журнал), но
считать их неверными в части «не закоммичено». Также в рамках подготовки к **E14-09**
(по разрешению владельца) пройден живой smoke: `.venv` backend довооружён
`faster-whisper` (тестов не было в этом venv), локальная sqlite домигрирована,
реальное видео взято read-only с прод-БД (`ssh sfera`) + прод-S3, полный путь воркера
(`claim_batch`→`_process_job`) реально скачал видео, распознал речь (faster-whisper) и
получил LLM-пересказ (через `OPENROUTER_API_KEY` как временную замену отсутствующего
`PROXY_API_KEY`/proxyapi.ru) — `TranscriptionJob` дошёл до `DONE` с корректными полями.
Тестовые записи удалены после прогона, прод/staging не изменялись (только `SELECT`).
Владелец подтвердил: боевого ключа proxyapi нет и не будет, в прод идёт OpenRouter
(`SUMMARY_MODEL=openai/gpt-4o-mini`, `PROXY_API_BASE=https://openrouter.ai/api/v1`) —
`step-10-prod-rollout.md` внешнего плана поправлен. По просьбе владельца
(«сначала закрыть 3 оставшиеся smoke-пункта») дожаты все 4 ручных smoke-пункта
живым прогоном без моков: тишина (синтетическое видео через ffmpeg, `is_empty=True`
без LLM-вызова), 5 кандидатов одновременно (`claim_batch`+`ThreadPoolExecutor`, без
дублей, все DONE за ~94с — с оговоркой: sqlite не даёт настоящей Postgres-семантики
`select_for_update`, проверена только прикладная логика), перезапуск воркера
(`reap_stale_jobs()` реально вернул зависший джоб в PENDING/довёл до DONE, и отдельно
до FAILED после лимита попыток). Все тестовые записи удалены, прод/staging не
менялись. **`step-09-tests-smoke.md` внешнего плана — все пункты DONE.** Следующий
шаг — **E14-09** (прод-роллаут), ждёт отдельного явного разрешения владельца на
конкретные правки в `sfera_backend` (Dockerfile/compose/env/деплой).

`2026-09-09` — шаг **E14-08** (тесты и smoke воркера
...
2026-09-08, архитектурное ревью тем же днём нашло 8 находок — все закрыты владельцем
2026-09-08/09 и внесены в текст дизайн-документа: (1) асинхронная постановка джоб
вместо синхронной в теле `POST vacancy-profile/` — подтверждён реальный масштаб до 500
кандидатов на демо-должность; (2) `pilot_course_id`-ограничение очереди сейчас не
выставлено, сервис ещё не в проде — не блокирует, но фильтрацию в `job_processing.py`
стоит поправить до первого боевого запуска на непилотном курсе; (3) отображение
`floor(fit_score/10)` вместо `round` — граница округления теперь совпадает с порогом
`fit_score>=60`; (4) экспорт помечает «передан» только кандидатов с реально собранным
`card.pdf`, не всех id из запроса; (5) upsert `CandidateVacancyTransfer` обновляет
`transferred_at` при повторной выгрузке (не «ничего не делает» — сняли внутреннее
противоречие дизайн-документа); (6) авторизация — SFERA-AI как был без per-user
проверки (только `bff_shared_secret`, сервис-к-сервису), так и остаётся; проверку прав
HR на курс переносит на себя **новый проксирующий endpoint в `sfera_backend`**
(`CourseCandidatesManagementPermission`, тот же класс, что уже защищает «Выгрузить
архив» — company-scoped, персональной привязки HR→курс в платформе нет и не
появляется). Проверено по коду FullSphera (`core/permissions.py:460`,
`courses/views.py` `CandidateArchiveJobCreateView` — образец паттерна). Интеграция
backend FullSphera → SFERA-AI сейчас отсутствует полностью (секрета/клиента нет) —
заводится с нуля шагами E15-06/07. Эпик — один (E15), шаги физически в трёх
репозиториях (SFERA-AI без ограничений; `sfera_backend`/`SPHERA` — явное разрешение
владельца на каждый шаг, по прецеденту E14). Реализация не начата.

`2026-09-09` — шаг **E14-06** (UI в карточке кандидата HR/Admin, код в `SPHERA`)
реализован по явному разрешению владельца. Тип `CandidateAnswerTranscription`
(`status`/`summary`/`transcript`/`isEmpty`, без `verdict`) в `candidate-model.ts`, маппинг в
`candidate-answers-mapper.ts`, новый компонент `TranscriptionBlock` в `CandidateProfile.tsx`
(бейдж статуса со спиннером на PENDING/PROCESSING, summary всегда на виду, транскрипт под
спойлером, пустое видео → понятный текст вместо пустоты). Без агрессивного поллинга —
данные обновляются при перезаходе на страницу. RBAC не менялся — эндпоинт и так HR/Admin-only
(E14-05). `npx tsc --noEmit` и `eslint` по правленым файлам — чисто. Не закоммичено. Детали —
журнал ниже и `step-E14-06-hr-ui.md` / `step-07-hr-ui.md` внешнего плана.

`2026-09-07` — шаг **E14-05** (транскрипт/summary
...
в `project-context/PLATFORM_video-transcription-plan/step-04/06/07/09/10`). Реализация
не начата — каждый шаг эпика E14 требует отдельного явного разрешения владельца (код
другого репозитория, правило `CLAUDE.md`).

`2026-09-06` — найдены и исправлены **три реальных бага**,
...
`feat/e17-04-06-card-visual-style` закоммичена и запушена, ожидает ручной проверки
владельца на реальных данных (см. E17-04/05/06 ниже).

- **E17-01** — резюме подменяет платформенные поля при успешном парсинге:
  `phone_number`/`city` добавлены в резюме-LLM извлечение
  (`resume_extraction.py:_SYSTEM_PROMPT`, `candidate_facts.py:_RESUME_FACT_KEYS`),
  новый `pick_fact_value` (`candidate_facts.py`) — приоритет источника
  (HH_RESUME/ANKETA_FILE > ANSWER > VIDEO, E13-01), `ai_card.py:_fact_value` —
  обёртка над ним.
- **E17-02** — баг невложения резюме (кейс Петрусевич 2398) уже был закрыт
  фиксом `company_slug` (`0af0089`, 2026-09-09) — подтверждено на staging,
  дополнительного кода не потребовалось.
- **E17-03** — резюме мерджится страницами в `card.pdf` вместо отдельного
  `resume.<ext>` в ZIP (новый `services/export/pdf_merge.py:merge_card_with_resume`,
  `pypdf.PdfWriter`, интеграция в `archive.py`). Решение владельца 2026-09-10 по
  не-PDF резюме (docx/картинка/др.): страница-заглушка в `card.pdf`
  («резюме приложено отдельным файлом, формат .<ext> не поддерживает
  предпросмотр», `reportlab`, тот же `DejaVuSans`/`_STYLES` что в `ai_card.py`) +
  сам файл резюме всё равно отдельно в ZIP как раньше (fallback). `card.pdf`
  недоступен (нет анализа) — резюме пишется отдельным файлом как прежде, merge
  не применяется. Тесты: новый `tests/services/test_pdf_merge.py` + 3 новых теста
  в `test_export_archive.py`. `uv run pytest` — 220 passed. Ничего не закоммичено.
- **E17-04** — новые факты резюме `age`/`industry`: `_SYSTEM_PROMPT` дополнен,
  `PROMPT_VERSION` → `resume-extract-v2`, `_RESUME_FACT_KEYS` дополнен. Тесты
  `resume_extraction`/`candidate_facts` — 18 passed. Ручная проверка на реальном
  резюме не проводилась.
- **E17-05** — формат ответа fit-scoring под эталон РТХ: `_SYSTEM_PROMPT`
  переформулирован (summary — список 3-5 предложений, strengths/
  interview_questions/contradictions — конкретные формулировки, criteria_scores
  — шкала 0-10 шаг 0.5 или null), `PROMPT_VERSION` → `fit-scoring-v3`, `summary`
  в `_LIST_FIELDS`, `CandidateVacancyAnalysis.summary` — колонка `JSON`
  (миграция `0009_fit_scoring_summary_list.py`). Ручная проверка на реальном
  кандидате не проводилась.
- **E17-06** — визуальный стиль карточки под эталон РТХ: `ai_card.py` — Letter,
  design tokens эталона (цвета/шрифты), квадратные буллеты по цвету колонки,
  `_platform_block` — буллеты из `summary` + "Важно:" из `contradictions`
  (дамп фактов убран), правая колонка — `interview_questions`, шапка —
  название заказчика (`resolve_company_name_for_course`, `api_read.py`,
  fallback "SFERA"), единый цвет баллов, формат `X,X/10`. Тесты — 222 passed,
  mypy чист. Визуальный прогон на синтетических данных — вёрстка без наложений.
  Бок-о-бок сравнение с PDF-эталоном РТХ на реальных кандидатах
  (`candidate_2964*`/`candidate_2623`/`candidate_3112`) не проводилось — файлы
  эталона недоступны в среде выполнения.

Следующая задача — ручная визуальная проверка E17-04/05/06 владельцем на реальных
кандидатах, затем мердж `feat/e17-04-06-card-visual-style`.

**Эпик E16 полностью завершён по коду** (2026-09-09, «Деплой сервиса на VPS») — E16-01…03
DONE. Единственный открытый пункт — реальный первый прогон `scripts/deploy-ai-service.sh` на
VPS (нужен доступ/разрешение владельца поднимать прод-контейнеры).
...
**Эпик E15 полностью завершён по коду** (2026-09-09, «Обработка кандидатов») —
E15-01…08 DONE во всех трёх репозиториях (SFERA-AI/`sfera_backend`/`SPHERA`).
Единственный открытый пункт — ручная браузерная проверка golden path/пустого списка
(DoD E15-08, «Как проверить») на dev-сервере SPHERA, не выполнена в этой сессии.
Ничего не закоммичено ни в одном из трёх репозиториев.
...
| E15-08 | HR UI модалка, код в `SPHERA` (`step-E15-08-hr-ui-modal.md`) | DONE (код+тесты; браузерная проверка не выполнена) | 2026-09-09 |
| E16-01 | `ai-scheduler` отдельным сервисом в docker-compose, единственный инстанс (`step-E16-01-scheduler-compose-service.md`) | DONE | 2026-09-09 |
| E16-02 | `scripts/deploy-ai-service.sh` (`step-E16-02-deploy-script.md`) | DONE (код; живой прогон на VPS не выполнялся) | 2026-09-09 |
| E16-03 | `docs/DEPLOYMENT.md` (`step-E16-03-deployment-doc.md`) | DONE | 2026-09-09 |
