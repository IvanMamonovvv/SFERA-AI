...
| E24 | Баг: BACKFILL-джобы новых кандидатов не считают fit_score (только факты) | E15 | средний — меняет ветку реального AI-вызова в очереди |
| E25 | Счётчик прогресса обработки в модалке HR-скрининга | E24 | часть шагов в других репозиториях — явное разрешение на каждый |
...
### E24 — Баг: BACKFILL не считает fit_score

Найдено 2026-09-10 архитектурным ревью при разборе плана E25 (счётчик прогресса). Владелец
попросил счётчик «осталось обработать N кандидатов» в модалке HR-скрининга (открытие
модалки, не realtime). Ревью показало: `remaining=0` будет врать — новые кандидаты (первый
отклик, ни разу не анализировались) обрабатываются только через `reason='BACKFILL'`, а
`_run_real_ai_call` (`job_processing.py:77-86`) для BACKFILL вызывает только
`build_or_update_candidate_facts` — `run_fit_scoring` не вызывается вовсе, т.к. `BACKFILL`
не входит в `VACANCY_REASONS = ("VACANCY_PROFILE_CHANGED", "FEEDBACK_APPLIED")`
(`job_processing.py:21`). Джоба закрывается `DONE`, но кандидат никогда не попадёт в список
подходящих (`fit_score>=60`) — не потому что не подошёл, а потому что его не считали. Уже
проанализированные кандидаты не затронуты — их пересчитывает отдельная синхронная джоба
`VACANCY_PROFILE_CHANGED` (`services/vacancy_profile.py:44` → `enqueue_fit_recalc_for_course`).
Предпосылка для E25 — без фикса счётчик показывает ложное «готово».

- [ ] `epics/E24-backfill-fit-scoring-bug/step-E24-01-backfill-runs-fit-scoring.md` —
  `_run_real_ai_call` реально считает `fit_score` для `reason='BACKFILL'`, не только
  факты. Решить: добавить `"BACKFILL"` в `VACANCY_REASONS` (job.course_id для BACKFILL
  всегда явный, `job_detection.py:104-127` — не нужен обходной путь через
  `application_id`), либо другой вариант — сверить на этапе реализации. Также решить,
  что делать с `FAILED`-джобами, исчерпавшими ретраи (`ai_processing_job_max_attempts`) —
  архитектурное ревью отметило, что они тоже невидимы как «незавершённые», отдельный
  вопрос от основного бага, но затрагивает тот же код.

### E25 — Счётчик прогресса обработки (модалка HR-скрининга)

По прямому запросу владельца (2026-09-10): после обновления портрета вакансии менеджер не
видит, идёт ли ещё обработка кандидатов и сколько осталось. Дизайн (из обсуждения): при
открытии модалки — не realtime — показать «осталось обработать N» / «обработка завершена».
Знаменатель = все открытые (`PENDING`/`PROCESSING`) `AIProcessingJob(reason='BACKFILL')`
курса, без привязки к конкретной версии портрета (осознанный трейдофф, не путать с
«прогресс именно этого сохранения» — если HR сохранил портрет повторно до завершения
предыдущей партии, счётчик покажет общий хвост, не разбитый по версиям). Список уже
подходящих (`fit_score>=60`) — существующий эндпоинт E15, показывается рядом. Зависит от
E24 — без него счётчик может показать «готово» при непосчитанных новых кандидатах.

**Блок A — SFERA-AI (свой репозиторий):**
- [x] `epics/E25-screening-progress-counter/step-E25-01-progress-endpoint.md` — новый
  `GET .../vacancy-profile/screening-progress/` (`api/routes/vacancy.py`, тот же паттерн
  `course_uuid` → `resolve_course_or_404`, что у соседних роутов этого файла — не raw
  `course_id`). Считает `AIProcessingJob` по `course_id`, `reason='BACKFILL'`:
  `remaining = count(status IN (PENDING, PROCESSING))`. Отдельно решить (по находке
  ревью) — включать ли в `remaining` `FAILED`-джобы с исчерпанными попытками, либо
  отдавать отдельным полем `failed`, чтобы не прятать зависшие навсегда джобы под видом
  «готово». Тесты: пустая очередь → 0; смесь статусов; изоляция по `course_id`;
  `FAILED` с исчерпанными попытками — отдельный кейс; джобы других `reason`/чужого курса
  не подмешиваются.

**Блок B — `sfera_backend` (другой репозиторий, разрешение на каждый шаг):**
- [x] `epics/E25-screening-progress-counter/step-E25-02-backend-proxy-endpoint.md` —
  проксирующий `GET` в `sfera_ai_proxy_views.py`, тот же `CourseCandidatesManagementPermission`,
  что у остальных screening-эндпоинтов E15. Новый клиент-метод
  `fetch_screening_progress` в `integrations/sfera_ai/client.py`.

**Блок C — `SPHERA` (фронтенд, разрешение на каждый шаг):**
- [x] `epics/E25-screening-progress-counter/step-E25-03-modal-progress-display.md` —
  `CandidateScreeningModal.tsx`: при открытии модалки запрос прогресса, показ «Осталось
  обработать: N» либо «Обработка завершена» рядом с уже существующим списком подходящих.
...
E15 → E24 (баг: BACKFILL не считает fit_score, весь код в SFERA-AI)
E24 → E25 (счётчик прогресса обработки; шаги в sfera_backend/SPHERA — отдельное разрешение владельца на каждый)
